...
resource "aws_acm_certificate" "cert" {
  domain_name       = "readyprep.io"
  validation_method = "DNS"

  subject_alternative_names = [ "app.readyprep.io" ]
...
resource "aws_acm_certificate_validation" "cert" {
  certificate_arn         = aws_acm_certificate.cert.arn
  validation_record_fqdns = [aws_route53_record.cert_validation.fqdn, aws_route53_record.cert_validation_app.fqdn]
}
...
resource "aws_ecs_task_definition" "app" {
  ...
  container_definitions = jsonencode([{
    ...
    secrets = [
      ...
      {
        "name": "CLOVER_APP_ID",
        "valueFrom": "arn:aws:ssm:us-east-1:${data.aws_caller_identity.current.account_id}:parameter/pos/clover/appId"
      },
      {
        "name": "CLOVER_APP_SECRET",
        "valueFrom": "arn:aws:ssm:us-east-1:${data.aws_caller_identity.current.account_id}:parameter/pos/clover/appSecret"
      },
      {
        "name": "CLOVER_WEBHOOK_CODE",
        "valueFrom": "arn:aws:ssm:us-east-1:${data.aws_caller_identity.current.account_id}:parameter/pos/clover/webhookCode"
      },
      {
        "name": "DATABASE_HOST",
      ...
      {
        "name": "SMTP_HOST",
        "valueFrom": "arn:aws:ssm:us-east-1:${data.aws_caller_identity.current.account_id}:parameter/smtp/host"
      },
      {
...
resource "aws_s3_bucket" "cloudfront-logs" {
  bucket = "readyprep-logs"
  acl    = "private"
}

resource "aws_cloudfront_distribution" "main" {

  aliases = [ "readyprep.io", "app.readyprep.io" ]
  enabled = true
...
resource "aws_route53_record" "old-dns" {
  zone_id = "Z0409763RXC6DK3EB098"
  name    = "readyprep.io"
  type    = "A"
  ttl = 300
